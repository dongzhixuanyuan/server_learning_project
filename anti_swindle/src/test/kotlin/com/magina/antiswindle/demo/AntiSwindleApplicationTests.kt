...
class AntiSwindleApplicationTests {
    ...
    fun testResItemInsert() {
        val itemResource = ItemResource(
            ...
            "深圳反诈", Data(
                ...
                "${env!!.BASE_VIDEO_DIR}8f7N5lt1lx07IGXucdKo01041200AOiX0E010.mp4", "blank"
            )
        )
    ...
    @Test
    fun testResItemUpdate(){

        val df = SimpleDateFormat("yyyy-MM-dd HH:mm:ss") //设置日期格式

        val date = df.format(Calendar.getInstance().time) // new Date()为获取当前系统时间，也可使用当前时间戳
        ...
        mainContentService!!.updateRes(itemResource)
    }

    @Test
    fun testResItemDelete(){
        mainContentService!!.deleteRes(5)

    }@Test
    fun testResItemQuery(){
        val queryRes = mainContentService!!.queryRes(7)
        println(queryRes)
